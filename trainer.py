...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, opt_group in enumerate(self.optimizer.param_groups):
            ...
            self.writer.add_scalar(f"{self.wrt_mode}/Momentum_{i}", opt_group["momentum"], self.wrt_step)
    ...
    def _valid_epoch(self, epoch):
        ...
        if self.val_loader is None:
            self.logger.warning("No data loader was passed for the validation step, No validation is performed !")
            return {}
        ...
        with torch.no_grad():
            ...
            for _batch_idx, (data, target) in enumerate(tbar):
            ...
            with io.BytesIO() as buffer:
                ...
                display = ConfusionMatrixDisplay(self.confusion_matrix.cpu().numpy()).plot()
                display.figure_.savefig(buffer, format="png")
                
                buffer.seek(0)
                image = Image.open(buffer)
                confusion_array = pil_to_tensor(image)

                self.writer.add_image(f"{self.wrt_mode}/confusion_matrix", confusion_array, self.wrt_step)
            ...
            for k, v in seg_metrics["Class_IoU"].items():
                self.writer.add_scalar(f"{self.wrt_mode}/Class_IoU_{k}", v, self.wrt_step)
    ...
    def _update_seg_metrics(self, correct, labeled, inter, union, conf_mat):
